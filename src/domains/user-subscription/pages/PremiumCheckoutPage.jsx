...
function PremiumCheckoutPage() {
  ...
  const selectedProduct = products.find((product) => product.id === subscriptionProductId) ?? products[0];
  ...
  return (
    <div className="mapingo-dashboard">
      <MapingoPageSection
        eyebrow="Payment"
        title="결제 처리"
        description="실결제 API 대신, 선택한 구독 상품과 결제 수단을 확인하고 프론트 단계에서 완료 상태를 검증하는 화면입니다."
      >
      ...
      <section className="mapingo-admin-grid">
        ...
        <article className="mapingo-list-card">
          ...
          <div className="mapingo-admin-form">
            <select className="mapingo-input" value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}>
            ...
            {isComplete ? (
              <div className="mapingo-empty-state">
                결제가 완료된 것으로 처리했습니다. 실제 서비스에서는 결제 API 응답과 영수증 번호가 함께 저장됩니다.
              </div>
            ) : null}
          </div>
        </article>
